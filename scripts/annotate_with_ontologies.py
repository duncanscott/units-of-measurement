...
def select_best_uo(record: dict, matches: List[UOTerm], norm_names: List[Optional[str]]) -> Optional[UOTerm]:
    ...
    if len(matches) == 1:
        ...
        if term.label_norm and term.label_norm in name_set:
            return term
        if _uo_matches_context(term, prop, quantity):
            return term
        return None
...
def load_om_terms(path: Path) -> tuple[Dict[str, List[OMTerm]], Dict[str, OMTerm]]:
    ...
    for tag in ("om:Unit", "om:PrefixedUnit"):
        for unit in root.findall(f".//{tag}", ns):
            uri = unit.attrib.get(f"{{{RDF_NS}}}about")
            if not uri:
                continue
            labels = []
            for lbl in unit.findall("rdfs:label", ns):
                lang = lbl.attrib.get(f"{{{XML_NS}}}lang") or "en"
            ...
            if not labels:
                continue
            alternative_labels = [node.text for node in unit.findall("om:alternativeLabel", ns) if node.text]
            comments = [node.text for node in unit.findall("rdfs:comment", ns) if node.text]
            definition = comments[0] if comments else None
            quantities: List[str] = []
            for rel in unit.findall("om:hasQuantity", ns):
            ...
            dimension = None
            dim = unit.find("om:hasDimension", ns)
            if dim is not None:
            ...
            for sym_tag in ("om:symbol", "om:alternativeSymbol", "om:LaTeXSymbol"):
                for node in unit.findall(sym_tag, ns):
                    if node.text:
